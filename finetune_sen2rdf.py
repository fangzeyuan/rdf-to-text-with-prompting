    ...
    def __init__(self, tokenizer: PreTrainedTokenizer, file_path: str, block_size: int, bos_tok: str, eos_tok: str,
        ...
        if "prefix" in opt.flag:
            ...
            for src, tgt in zip(src_lines, tgt_lines):
                sent = ' {} {} {} '.format(additional_special_tokens[0], tgt, bos_tok) + src + ' {}'.format(eos_tok)
                edited_sents.append(sent)
        else:
            edited_sents = []
            for src, tgt in zip(src_lines, tgt_lines):
                sent = ' {} {} '.format(tgt, bos_tok) + src + ' {}'.format(eos_tok)
                edited_sents.append(sent)

        if opt.sen2rdf_augment_data_tag == True:
            ...
            for index, row in pd_reader.iterrows():
                ...
                if str1.endswith("."):
                    ...
                    if str1 != 'nan' and str1 != 'None' and str1 != '':
                        if "prefix" in opt.flag:
                            sent = ' {} {} {} '.format(additional_special_tokens[0], str1, bos_tok) + str3 + ' {}'.format(
                                eos_tok)
    ...
    def __init__(self, tokenizer: PreTrainedTokenizer, file_path: str, block_size: int, bos_tok: str, eos_tok: str,
        ...
        if opt.sen2rdf_augment_data_tag == True:
            if opt.augment_tag == "sts-roberta":
                ...
                pd_reader = pd.read_csv(opt.evalaute_dart_output_files_prefix_test_file_final)
            for index, row in pd_reader.iterrows():
                str1 = str(row['bleu_pred'])
                if str1.endswith("."):
                    if str1[0] != '|':
        ...
        elif opt.webnlg_sen2rdf_augment_data_tag == True:
            if opt.augment_tag == "sts-roberta":
    ...
    def __init__(self, tokenizer: PreTrainedTokenizer, file_path: str, block_size: int, bos_tok: str, eos_tok: str,
        ...
        if opt.sen2rdf_augment_data_tag == True:
            if opt.augment_tag == "sts-roberta":
                ...
                pd_reader = pd.read_csv(opt.evalaute_webnlg_output_files_prefix_test_file_final)
            for index, row in pd_reader.iterrows():
                str1 = str(row['bleu_pred'])
                if str1.endswith("."):
                    if str1[0] != '|':
                    ...
                    if str1 != 'nan' and str1 != 'None' and str1 != '':
                        ...
                        full_tgt_lst.append(str3)
                        full_rela_lst.append(rela_list2)
        elif opt.dart_sen2rdf_augment_data_tag == True:
            if opt.augment_tag == "sts-roberta":
                pd_reader = pd.read_csv(opt.evalaute_dart_output_files_sts_roberta_test_file)
            elif opt.augment_tag == "vae":
                pd_reader = pd.read_csv(opt.evalaute_dart_output_files_vae_test_file)
...
def train(args, train_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer) -> Tuple[int, float]:
    ...
    if opt.flag in ["finetune_gpt2_additional_layer", "finetune_gpt2_additional_layer_with_prefix"]:
        ...
        model = finetune_gpt2_vae.VariationalAutoencoder(device=args.device, model=model, tokenizer=tokenizer,
                                                         latent_dims=32)
        model.to(args.device)
    ...
    if opt.flag == "finetune_gpt2" or opt.flag == "finetune_gpt2_prefix":
        ...
        model.decoder.resize_token_embeddings(len(model.tokenizer))
    elif opt.flag in ["finetune_gpt2_vae", "finetune_gpt2_vae_with_prefix"]:
        model.decoder.decoder.resize_token_embeddings(len(model.decoder.tokenizer))
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if opt.flag == "finetune_gpt2" or opt.flag == "finetune_gpt2_prefix":
                ...
                if 't5' in opt.model_type:
                    ...
                    input_ids = tokenizer.batch_encode_plus(input_data, padding=True, truncation=True, max_length=512,
                                                            return_tensors='pt')['input_ids'].to(args.device)
                    labels = tokenizer.batch_encode_plus(label_sents, padding=True, truncation=True, max_length=512,
                                                         return_tensors='pt')['input_ids'].to(args.device)
                ...
                model.train()
                loss = model(inputs=rdfs, labels=labels, samples=samples, attention_mask=attention_mask)
...
def bleu_score_probing(model, tokenizer):
    ...
    generation = generate_sen2rdf.generate_predictions_for_bleu_score_probing(model, tokenizer)
...
def finetune(paras):
    ...
    special_tokens_dict = {'additional_special_tokens': ['<sen2rdf>']}
    num_added_toks = tokenizer.add_special_tokens(special_tokens_dict)
...
if __name__ == '__main__':
    ...
    # test_data_path = opt.rdf2sen_aug_data_filter
    #
    # if "webnlg" in opt.train_data_file:
    #     test_data_path = opt.webnlg_files_sts_roberta_test_file
    # elif "DART" in opt.train_data_file:
    #     test_data_path = opt.dart_files_sts_roberta_test_file
    # elif "e2e" in opt.train_data_file:
    #     test_data_path = opt.e2e_files_sts_roberta_test_file
    #
    # if "DART" in opt.train_data_file and opt.augment_tag == 'sts-roberta':
    #     output_path = opt.dart_output_files_sts_roberta_test_file
    # elif "DART" in opt.train_data_file and opt.augment_tag == 'vae':
    #     output_path = opt.dart_output_files_vae_test_file
    # elif "DART" in opt.train_data_file and opt.augment_tag == 'prefix':
    #     output_path = opt.dart_output_files_prefix_test_file
    # elif "webnlg" in opt.train_data_file and opt.augment_tag == 'sts-roberta':
    #     output_path = opt.webnlg_output_files_sts_roberta_test_file
    # elif "webnlg" in opt.train_data_file and opt.augment_tag == 'vae':
    #     output_path = opt.webnlg_output_files_vae_test_file
    # elif "webnlg" in opt.train_data_file and opt.augment_tag == 'prefix':
    #     output_path = opt.webnlg_output_files_prefix_test_file
    # elif "e2e" in opt.train_data_file and opt.augment_tag == 'sts-roberta':
    #     output_path = opt.e2e_output_files_sts_roberta_test_file
    # elif "e2e" in opt.train_data_file and opt.augment_tag == 'prefix':
    #     output_path = opt.e2e_output_files_prefix_test_file
    # elif "e2e" in opt.train_data_file and opt.augment_tag == 'vae':
    #     output_path = opt.e2e_output_files_vae_test_file
    # else:
    #     output_path = opt.output_path
    #
    # generate_sen2rdf.generate(model_type=opt.model_type,
    #                           model_path=opt.model_path,
    #                           test_data_path=test_data_path,
    #                           logging_steps_generate=opt.logging_steps_generate,
    #                           sts_roberta_generated_sent=True,  # If sts_roberta_generated_sent is True, then we used the generated sentences from sts_roberta model. Otherwise, it is a normal sentence to rdf task.
    ...
    generate_sen2rdf.generate(model_type=opt.model_type,
                              ...
                              logging_steps_generate=opt.logging_steps_generate,
                              sts_roberta_generated_sent=False,
                              # If sts_roberta_generated_sent is True, then we used the generated sentences from sts_roberta model. Otherwise, it is a normal sentence to rdf task.
                              output_path=opt.output_path)
