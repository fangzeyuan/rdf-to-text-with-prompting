...
def get_self_bleu_result(candidates):
    # print(candidates)
    avg_score = metrics.self_bleu_score(candidates)

    return {"self-bleu": avg_score}


def get_hard_coverage_score(input_text, prediction, text2data=False):
    translator = str.maketrans(string.punctuation, ' ' * len(string.punctuation))
    cov_score = 0
    if text2data:
        gen_pairs = prediction.replace('<BOS>', '').replace('<|endoftext|>', '').strip().split('|')
        gen_pairs = list(filter(None, gen_pairs))
        input_str = " ".join(input_text.replace('<BOS>', '').replace('<|endoftext|>', '').strip().translate(translator).split()).strip().lower()
        gen_str_li = ' '.join(gen_pairs).translate(translator).strip().lower().split()
        input_text_li = input_str.split()

        gen_str_li = [g for g in gen_str_li if g not in stopwords.words('english')]
        input_text_li = [i for i in input_text_li if i not in stopwords.words('english')]

        if len(input_text_li) < 1:
            return {"coverage-score_pred": 0.0}
        else:
            cov_score = round(len(set(gen_str_li) & set(input_text_li)) /len(input_text_li), 3)
    else:
        input_pairs = input_text.replace('<BOS>', '').replace('<|endoftext|>', '').strip().split('|')
        input_pairs = list(filter(None, input_pairs))
        prediction = " ".join(prediction.translate(translator).split()).strip().lower()
        val_list = []
        for inp in input_pairs:
            ...
            if len(items) < 2:
                ...
                val_list.append(items[1].strip().lower())
            else:
                val_list.append(" ".join(items[0].translate(translator).split()).strip().lower())
                val_list.append(" ".join(items[-1].translate(translator).split()).strip().lower())
        if len(val_list) < 1:
            return {"coverage-score_pred": 0.0}
        for inp in val_list:
            if inp in prediction:
                cov_score += 1
        cov_score = round(cov_score/len(val_list), 3)

    return {"coverage-score_pred": cov_score}
...
def corpus_score(input_text, target_text, predictions, text2data=False):
    ...
    # results.update(get_self_bleu_result(predictions))
    # results.update(get_grammatical_error(predictions))
    results.update(get_hard_coverage_score(input_text, bleu_pred['bleu_pred'],text2data=text2data))
