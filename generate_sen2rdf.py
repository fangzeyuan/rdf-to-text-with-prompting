...
def read_DART_files(path, tokenizer):
    ...
    for src, tgt in zip(full_src_lst, full_tgt_lst):
        sent = ' {} {}'.format(tgt, tokenizer.bos_token)
        if sent not in file_dict:
            file_dict[sent] = []
        file_dict[sent].append(src)
    return file_dict
...
def read_files_from_CSV(path, tokenizer):
    pd_reader = pd.read_csv(path)
    file_dict = {}
    for index, row in pd_reader.iterrows():
        if row['bleu_pred'] == row['rouge_pred']:
            sent = ' {} {}'.format(row['bleu_pred'], tokenizer.bos_token)
            if sent not in file_dict:
                file_dict[sent] = []
            file_dict[sent].append(row['input'])
        elif row['bleu_pred'] != row['rouge_pred']:
            item_list = [row['bleu_pred'],row['rouge_pred']]
            for item in item_list:
                sent = ' {} {}'.format(item, tokenizer.bos_token)
                if sent not in file_dict:
                    file_dict[sent] = []
                file_dict[sent].append(row['input'])
...
# Created for testing BLEU Scores during the evaluate() step in finetune_sen2rdf.py
...
def generate(model_type: str, model_path: str, test_data_path: str, logging_steps_generate: str, sts_roberta_generated_sent: bool, output_path: str):
    ...
    if opt.flag == "finetune_gpt2" or opt.flag == "finetune_gpt2_prefix":
        ...
        model = load_checkpoint(load_path=model_path, model=model, device=device)
    if "e2e" in opt.train_data_file and sts_roberta_generated_sent is False:
        test_dataset_dict = read_e2e_files(test_data_path, tokenizer)
    elif "e2e" in opt.train_data_file and sts_roberta_generated_sent is True:
        test_dataset_dict = read_files_from_CSV(test_data_path, tokenizer)
    elif "DART" in opt.train_data_file and sts_roberta_generated_sent is False:
        test_dataset_dict = read_DART_files(test_data_path, tokenizer)
    elif "DART" in opt.train_data_file and sts_roberta_generated_sent is True:
        test_dataset_dict = read_files_from_CSV(test_data_path, tokenizer)
    elif "webnlg" in opt.train_data_file and sts_roberta_generated_sent is False:
        test_dataset_dict = read_webnlg_files(test_data_path, tokenizer)
    elif "webnlg" in opt.train_data_file and sts_roberta_generated_sent is True:
        test_dataset_dict = read_files_from_CSV(test_data_path, tokenizer)
...
def get_model_generation(sample, label, model, tokenizer, num_return_sequences=opt.num_return_sequences):
    ...
    if type(sample) == str:
        sample = [sample]
    gen_re = []
    for n, m in zip(sample, generation):
        int_to_rmv = n.replace(tokenizer.bos_token, '').replace(' .', '.').replace(' ,', ',').replace(' \'', '\'').strip()
        for m_i in m:
