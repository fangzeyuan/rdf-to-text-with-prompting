...
def load_and_cache_examples(args, tokenizer, evaluate=False):
    ...
    if args.line_by_line:
        ...
        for sample in data_samples:
            src_text.append(sample[1] + ' ' + sample[0])
        data_set = tokenizer.batch_encode_plus(src_text, pad_to_max_length=True)
        sample = [[torch.tensor(data_set["input_ids"][ind])] for ind in range(len(data_set["input_ids"]))]
...
def train(args, train_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer) -> Tuple[int, float]:
    ...
    def collate(sample: List[torch.Tensor]):
        ...
        if tokenizer._pad_token is None:
            text = pad_sequence(examples, batch_first=True)
            return text
        text = pad_sequence(examples, batch_first=True, padding_value=tokenizer.pad_token_id)
        return text
