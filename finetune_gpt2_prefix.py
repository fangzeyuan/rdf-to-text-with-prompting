...
def load_and_cache_examples(args, tokenizer, evaluate=False):
    ...
    if args.line_by_line:
        ...
        for sample in data_samples:
            # src_text.append(tokenizer.bos_token + sample[1] + sample[0] + tokenizer.eos_token)
            src_text.append(sample[1] + ' RDF ' + sample[0])
        data_set = tokenizer.batch_encode_plus(src_text, return_tensors="pt", padding=True)
        sample = data_set["input_ids"]
...
def train(args, train_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer) -> Tuple[int, float]:
    ...
    def collate(sample: List[torch.Tensor]):
        # examples = [item for item in sample]
        if tokenizer._pad_token is None:
            text = pad_sequence(sample, batch_first=True)
            return text
        text = pad_sequence(sample, batch_first=True, padding_value=tokenizer.pad_token_id)
        return text
...
if __name__ == '__main__':
    paras = ["--train_data_file", opt.train_data_file,
             ...
             "--do_train",
             "--do_eval",
             "--output_dir", opt.output_dir_predix,
    ...
    generate.generate(model_type=opt.model_type,
             model_path=opt.model_path_prefix,
             test_data_path=opt.test_data_path,
             batch_size=opt.batch_size,
             output_path=opt.output_path_prefix)
    print("finished")
